...
def null_dept
  ...
  SELECT
    t.name
  FROM 
    teachers t LEFT JOIN depts d ON t.dept_id = d.id
  WHERE 
    t.dept_id IS NULL
  SQL
end
...
def all_depts_join
  ...
  SELECT
    t.name, 
    d.name
  FROM depts d LEFT JOIN teachers t ON t.dept_id = d.id
  SQL
end
...
def num_teachers_and_mobiles
  ...
  SELECT
    COUNT(name),
    COUNT(mobile) --COALESCE(mobile, '07986 444 2266')
  FROM 
    teachers 
  SQL
end
...
def dept_staff_counts
  ...
  SELECT
    ...
    COUNT(t.name)
  FROM
    depts d LEFT JOIN teachers t ON d.id = t.dept_id
  GROUP BY 1
  SQL
end
