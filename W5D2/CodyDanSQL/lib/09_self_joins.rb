...
def lrt_stops
  ...
  SELECT
    ...
    s.name
  FROM
    stops s JOIN routes r ON s.id = r.stop_id
  WHERE 
...
def cl_to_lr
  ...
  WHERE
    a.stop_id = 53
    AND b.stop_id = (SELECT id FROM stops 
...
def start_at_craiglockhart
  ...
  JOIN
    routes b ON (a.company = b.company AND a.num = b.num)
  JOIN stops c ON a.stop_id = c.id
  JOIN stops d ON b.stop_id = d.id
  WHERE
...
def craiglockhart_to_sighthill
  ...
  JOIN stops d ON a.stop_id = d.id
  JOIN stops e ON b.stop_id = e.id
  WHERE 
    d.name = 'Craiglockhart' 
  SQL
end
